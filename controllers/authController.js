...
exports.signinController = (req, res) => {
  ...
  exports.googleLoginController = (req, res) => {
      ...
      //varify token from body
      client.varifyIdToken({
        idToken,
        audiences: process.env.GOOGLE_CLIENT
      })
      .then(responce => {
        const { 
          email_varified,
        ...
        //check if email varified
        if(email_varified){
          User.findOne({ email }).exec((err, user) => {

            //find if email alrady exists
            //if exists
            if(user){
              ...
              //second responce to client side token and user info
              return res.json({
                token,
              ...
              //if user not exist data save in database and create password
              let password = email + process.env.JWT_SECRET
              
              //create new object with this email
          ...
          //if anything wrong
          return res.status(400).json({
            error: 'Google login not working, please do it agin'
          })
